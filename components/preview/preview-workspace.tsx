    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <nav className="flex w-[794px] items-center gap-px rounded-full border border-line bg-white/90 p-1 shadow-card">
        ...
        <div className="flex items-center gap-1 pr-1">
          <span className={`rounded-full border px-3 py-1.5 text-[10px] font-medium tracking-wider transition ${
            realPageCount <= 1
              ? (squishLevel === 0 ? "border-emerald-100 bg-emerald-50/50 text-emerald-700" : "border-blue-100 bg-blue-50/50 text-blue-700")
              : "border-rose-100 bg-rose-50/50 text-rose-700"
          }`}>
            {realPageCount <= 1
              ? (squishLevel === 0 ? "1 页 A4 (适合投递)" : "1 页 A4 (已自动压缩排版)")
