...
export function TemplateProfessionalCN({ document }: TemplateProfessionalCNProps) {
  ...
  return (
    <div className="w-full text-[#1F2430] antialiased" style={{ fontFamily: '"Source Han Sans SC", "Noto Sans CJK SC", "PingFang SC", "Microsoft YaHei", sans-serif' }}>
      <header className="resume-header flex items-start gap-5 border-b-2 border-[#1F2430] pb-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline gap-3">
            <h2 className="text-[25pt] font-extrabold leading-none tracking-[0.02em] text-[#1F2430]">
              {document.header.name}
            </h2>
            <span className="text-[12.5pt] font-medium text-[#1F2430]">
              {document.header.title}
            </span>
          </div>

          {headerInfo.length > 0 && (
            <div className="contact-line mt-2 flex flex-wrap items-center gap-x-2.5 gap-y-0.5 text-[9.5pt] leading-[1.35] text-[#5B6472]">
              {headerInfo.map((item, i) => (
                <span key={i} className="inline-flex items-center whitespace-nowrap">
                  {item}
                  {i < headerInfo.length - 1 && (
                    <span className="ml-2.5 font-light text-[#CCC]">|</span>
                  )}
                </span>
              ))}
            </div>
          ...
          <section key={idx} className="resume-section break-inside-avoid pb-2.5">
            ...
            <ul className="resume-section-items flex flex-col gap-1.5 list-none">
              {section.items.map((item, itemIdx) => {
                if (item.type === "text") {
                  return (
                    <li key={itemIdx} className="text-item text-[10pt] leading-[1.5] text-[#1F2430]">
                      {item.text}
                    </li>
                  );
                }
