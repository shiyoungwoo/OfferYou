...
function renderStatusLabel(status: MasterInsightSummary["status"]) {
  switch (status) {
    case "confirmed":
      return "已确认";
          ...
          insights.map((insight) => (
            <article key={insight.id} className="rounded-[1.4rem] border border-line bg-paper p-5">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="text-xs uppercase tracking-[0.24em] text-accent">{renderStatusLabel(insight.status)}</span>
              </div>
