  ...
  return (
    <section className="rounded-[1.75rem] border border-line bg-white/85 p-6 shadow-card">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Insight Candidates</p>
          <h2 className="mt-3 text-2xl font-semibold">Insight Master</h2>
        </div>
        <div className="rounded-full border border-line px-4 py-2 text-sm text-slate-600">{insights.length} cards</div>
      </div>
        ...
        {insights.map((insight) => (
          <article key={insight.id} className="rounded-[1.4rem] border border-line bg-paper p-5">
            ...
            <p className="mt-4 text-xs uppercase tracking-[0.24em] text-slate-500">
              Evidence: {insight.evidenceLabels.join(" / ")}
            </p>
          </article>
        ))}
      </div>
