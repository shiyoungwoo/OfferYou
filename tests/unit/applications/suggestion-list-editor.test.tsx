...
describe("SuggestionList editor", () => {
  it("expands a suggestion, shows the quality hint separately, and allows local edits", () => {
    ...
    fireEvent.change(screen.getByLabelText("建议改成"), {
      target: {
        value: "Built AI workflow systems with stronger product framing."
      }
    });
    fireEvent.change(screen.getByLabelText("改写理由"), {
      target: {
        value: "把重点放在工作流设计和岗位匹配上。；质量提示：改写内容偏短。"
      }
    });

    expect(screen.getByDisplayValue("Built AI workflow systems with stronger product framing.")).toBeTruthy();
    expect(screen.getByDisplayValue("把重点放在工作流设计和岗位匹配上。；质量提示：改写内容偏短。")).toBeTruthy();
  });
});
