...
## 核心任务
根据 JD 和候选人的经历块，给出「改写前 / 改写后」的改写建议。`after` 字段必须是可以直接进入简历的完整改写内容，不输出分析、建议、占位符或省略号。改写必须明显贴合 JD，而不是复述原文。
...
- 学校 + 时间 + 专业 + 学历，一行即可。
- 禁止列课程表。
- 教育背景不生成改写建议，除非用户明确要求修正教育信息。

#### 技能与补充
- 只保留与目标岗位强相关的技能，分组展示：
  - 产品与分析 / AI 工具 / 协作与交付 / 语言与证书。
- 技能与补充不作为独立改写建议输出；如必须保留，只在最终简历中压缩为一行。
...
5. **岗位痕迹**：必须从 JD 中提取真实要求，围绕其中 1 到 3 个要求重组表达，如 AI 工作流、AIGC 内容、Prompt、产品迭代、数据分析、学习落地、跨团队协作等。不能使用「目标岗位要求的动作」「结果和协作方式」这类泛化占位词。
6. **完整输出**：`after` 必须是可直接放进简历的最终正文，严禁出现 `...`、`…`、`待补充`、`可写成`、`建议改为`、`请补充`、`这段经历`、`相关性较弱`、`可以`、`建议` 等元话语，也不能复制原文作为结果。如果信息不足，就写一版保守但完整的真实表达，低相关经历可以压缩，但不能空写。
  ...
  "suggestions": [
    {
      ...
      "title": "本条建议的简短标签",
      "before": "候选人原文（逐字引用）",
      "after": "改写后版本（完整正文，符合上述规则，不虚构，不写省略号，不写建议语气）",
...
禁止输出教育背景、技能与证书、补充信息作为 suggestions。禁止在 `after` 中出现「建议补充」「这段经历相关性较弱」「可写成」「可以改为」等元话术。
